...
export const AppHeader: React.FC<AppHeaderProps> = ({ userProfile, onOpenProfile }) => {
  const firstName = userProfile?.firstName?.trim() || "Alex";
  const lastName = userProfile?.lastName?.trim() || "Lindell";
  const role = userProfile?.role?.trim() || "Sales Manager";
  const companyName = userProfile?.companyName?.trim() || "Noir Hôtel Stockholm";
